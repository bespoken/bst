...
class BespokeClient {
    ...
    connect() {
        ...
        this.client.connect(this.port, this.host, function () {
            logging_helper_1.LoggingHelper.info(Logger, self.host + ":" + self.port + " Connected");
        ...
        this.onWebhookReceived = function (socket, request) {
            let self = this;
            logging_helper_1.LoggingHelper.info(Logger, self.nodeID + " onWebhook: " + request.toString());
            ...
            tcpClient.transmit("localhost", self.targetPort, request.toTCP(), function (data, error, message) {
                ...
                else if (error === global_2.NetworkErrorType.CONNECTION_REFUSED) {
    ...
    keepAlive() {
        logging_helper_1.LoggingHelper.debug(Logger, "KeepAlive PING");
        this.send(global_1.Global.KeepAliveMessage);
        this.keepAliveTimer();
    }
    send(message) {
